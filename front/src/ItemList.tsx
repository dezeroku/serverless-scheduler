    ...
    <div className="ItemList">
      {props.items.length !== 0 ? (
        <div className="Items m-2">
          <ListGroup>{renderItems(props.items, props.visibleCount)}</ListGroup>
        </div>
      ) : (
        <div>
      ...
      <div className="Controls m-2 mt-1">
        <ToggleButtonGroup
          type="radio"
          onChange={(val: number) => setTab(val)}
          name="tab"
          defaultValue={0}
        >
      ...
      <EditModal
        show={showEditModal}
        handleTask={(json: ItemProps["props"]) => props.handleUpdate(json)}
        onHide={() => {
          setShowEditModal(false);
        }}
        item={activeItem}
        editMode={true}
        handleDelete={(json: ItemProps["props"]) => props.handleDelete(json)}
